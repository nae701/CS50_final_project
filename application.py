...
def index():
    ...
    stocks = db.execute(
        "SELECT symbol, name, SUM(shares) FROM transactions WHERE user_id = :user_id GROUP BY symbol", user_id=session["user_id"])
...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("UPDATE users SET cash = cash - :cost WHERE id = :id",
                   cost=lookup(symbol)["price"] * int(shares), id=session["user_id"])
        return redirect('/')
    else:
...
def sell():
    ...
    if request.method == "POST":
        ...
        stock = db.execute("SELECT SUM(shares) FROM transactions WHERE user_id = :user_id AND symbol = :symbol",
        ...
        db.execute("UPDATE users SET cash = cash + :sale WHERE id = :id",
                   sale=lookup(symbol)["price"] * shares, id=session["user_id"])
        return redirect("/")
    else:
